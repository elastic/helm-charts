...
| `secretCreate`            | Allows you easily create a secret from as variables and from file. For add secrets from file, add suffix `.filepath` to the key of secret key. Useful for store certificates and other secrets. See [values.yaml][] for an example   | `[]`                                  |
