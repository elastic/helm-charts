...
versions = {
    6: os.environ.get("BUMPER_VERSION_6", "6.8.13-SNAPSHOT"),
}
...
for major, version in versions.iteritems():
    ...
    for pattern in goss_files:
        for f in glob.glob(pattern):
            print(f)
            for line in fileinput.input([f], inplace=True):
                # If we have a version with a build id, like 6.8.11-abcdabcd,
                # strip off the latter part and only use the 6.8.11 in the goss
                # tests
